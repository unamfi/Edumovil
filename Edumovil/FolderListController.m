...
- (void)viewWillAppear:(BOOL)animated {
	...
	__weak FolderListController *weakSelf = self;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (!_moving) {
		...
		if (info.isFolder) {
			controller = [[FolderListController alloc] initWithFilesystem:_filesystem root:info.path];
		} else {
	...
	if (!_creatingFolder) {
		...
		if (!success) {
			...
			FolderListController *controller =
				[[FolderListController alloc] initWithFilesystem:_filesystem root:path];
			[self.navigationController pushViewController:controller animated:YES];
		}
	}
}
...
- (UITableViewCell *)loadingCell {
	if (!_loadingCell) {
		...
		[_loadingCell.textLabel setTextAlignment:NSTextAlignmentCenter];
	}
	return _loadingCell;
}
